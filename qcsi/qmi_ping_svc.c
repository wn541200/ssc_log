...
//added by wei-hu
qmi_client_handle tct_log_clnt;
int tct_log_enable = 0 ;
struct loglist {
	char log[1024];
	int length;
...
struct loglist loghead;
qurt_mutex_t log_lock;
test_data_ind_msg_v01 *ind_log;
qurt_signal_t log_signal;
qurt_thread_t qmi_ping_service_loop_tcb;
qurt_thread_attr_t   qmi_ping_service_loop_attr;
char *qmi_ping_service_loop_stack = NULL;
#define QMI_SVC_LOG_WAIT_SIG 0x00020000
...
qmi_csi_cb_error send_log_to_ap(char *buf,unsigned int length)
{
	
	if(!tct_log_enable){
		MSG_1(MSG_SSID_SNS,MSG_LEGACY_FATAL,"QMI_ping_svc:send_log_to_ap:tct_log_enable=%d",tct_log_enable);
		return -1;
	}
	if(!buf)
	{
		MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"QMI_ping_svc:send_log_to_ap:buf is null");
		return -1;
	}
	if(length>1024||length<0){
		MSG_1(MSG_SSID_SNS,MSG_LEGACY_FATAL,"QMI_ping_svc:send_log_to_ap:length=%d",length);
		return -1;
	}
	struct loglist *node;
	qurt_mutex_lock(&log_lock); 
	node = (struct loglist*)MALLOC(sizeof(struct loglist));
	if(!node)
  {
  	
	MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"QMI_ping_svc:send_log_to_ap:QMI_CSI_CB_NO_MEM");
  	qurt_mutex_unlock(&log_lock); 
    return QMI_CSI_CB_NO_MEM;
  }
	QMI_MEM_COPY(node->log,sizeof(node->log),buf,length);
	node->length = length;
	list_add_tail(&node->list,&loghead.list);
	loghead.num += 1;
	qurt_mutex_unlock(&log_lock); 
	
	MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"QMI_ping_svc:send_log_to_ap:succeed");
	qurt_signal_set(&log_signal, QMI_SVC_LOG_WAIT_SIG);
	return 0;
}


static void loop_log(void * unused)
{
	qmi_csi_error resp_err;
	struct loglist *node;
	do{
		qurt_signal_wait_any(&log_signal, QMI_SVC_LOG_WAIT_SIG);
		qurt_signal_clear(&log_signal, QMI_SVC_LOG_WAIT_SIG);
		...
		if(!ind_log)
			{
				MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"QMI_ping_svc:loop_log:ind_log ins null");
				continue;
			}

		while(loghead.num>0){
			memset( ind_log, 0, sizeof(test_data_ind_msg_v01));		
			qurt_mutex_lock(&log_lock); 
			
			node=container_of(loghead.list.next,struct loglist,list);
			if(!node)
			{
				MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"QMI_ping_svc:loop_log:node is null");
				qurt_mutex_unlock(&log_lock);
				continue;
			}
			ind_log->data_len=node->length;
			QMI_MEM_COPY( ind_log->data, sizeof(ind_log->data),node->log, node->length);
			list_del(loghead.list.next);
			FREE(node);
			loghead.num--;
			qurt_mutex_unlock(&log_lock);
			#if 1
			resp_err = qmi_csi_send_ind( tct_log_clnt, QMI_TEST_DATA_IND_V01, ind_log, sizeof(test_data_ind_msg_v01) );
			if(resp_err != QMI_CSI_NO_ERR)
			{
				MSG_1(MSG_SSID_SNS,MSG_LEGACY_FATAL,"Fail:QMI_ping_svc:send_log_to_ap:resp_err = %d",resp_err);
			}
			#endif
...
static qmi_csi_cb_error send_indications(client_info_type *clnt)
{
  ...
  ind_type *params = NULL;
	MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"QMI_ping_svc:send_indications");
  if(!clnt)
  ...
  )
{
  ...
  MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"wei-hu-ping_data_response");
  ...
  )
{
  ...
  MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"wei-hu-ping_data_ind_registration");
  if(!req_c_struct)
  {
    return QMI_CSI_CB_INTERNAL_ERR;
  }
  ...
  )
{
  ...
  //MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"wei-hu-ping_connect");

  if(!connection_handle)
    return QMI_CSI_CB_CONN_REFUSED;
    //added by wei-hu
   MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"Qmi_ping_svc:ping_connect");
  tct_log_enable = 1;
  tct_log_clnt=client_handle;
 ...
 )
{
  ...
  MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"Qmi_ping_svc:ping_disconnect");
  if(connection_handle)
...
void *qmi_ping_register_service(qmi_csi_os_params *os_params)
{
  ...
  INIT_LIST_HEAD(&loghead.list);
  loghead.num = 0;
  qurt_mutex_init(&log_lock);
  ind_log = (test_data_ind_msg_v01*)CALLOC(1, sizeof(test_data_ind_msg_v01));
  if(!ind_log)	
	MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"QMI_ping_Svc:QMI_CSI_CB_NO_MEM");
  qurt_signal_init(&log_signal);
  ...
  if(rc != QMI_CSI_NO_ERR)
    return NULL;
#if 1
	qurt_thread_attr_init(&qmi_ping_service_loop_attr);
  qurt_thread_attr_set_name(&qmi_ping_service_loop_attr, "QMI_PING_SVC_LOOP");
  qurt_thread_attr_set_priority(&qmi_ping_service_loop_attr, 10);
  qurt_thread_attr_set_stack_size(&qmi_ping_service_loop_attr, QMI_PING_SERVICE_STACK_SIZE);
  qmi_ping_service_loop_stack = malloc(QMI_PING_SERVICE_STACK_SIZE);
  if(!qmi_ping_service_loop_stack)
  {
    /* This might be too early in boot for print messages */
		
		MSG(MSG_SSID_SNS,MSG_LEGACY_FATAL,"QMI_ping_Svc:qmi_ping_service_loop_stack error");
    goto fail;
  }
	qurt_thread_attr_set_stack_addr(&qmi_ping_service_loop_attr, qmi_ping_service_loop_stack);
	qurt_thread_create(&qmi_ping_service_loop_tcb, &qmi_ping_service_loop_attr,loop_log, NULL);
	fail:
	#endif

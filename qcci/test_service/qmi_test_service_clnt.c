...
static void parse_command(int argc, char *const argv[])
{
  ...
  while ((command = getopt_long(argc, argv, "qht:s:i:n:d:", longopts, NULL)) != -1)
  {
    switch (command)
    {
      case 'q':
...
)
{
  qmi_client_error_type err;
	test_data_ind_msg_v01 ind_msg1;
  --pending_inds;
  err = qmi_client_message_decode(user_handle,
	 ...
	 if(err != QMI_NO_ERR) {
           printf("\nError1: qmi_client_message_decode %d\n", err);
   }
	printf("Indication: msg_id=0x%x buf_len=%d,ind_data=%s,length=%d,pending_ind=%d\n", msg_id,ind_buf_len,ind_msg1.data,ind_msg1.data_len,pending_inds);
  //printf("TEST_SERVICE: Indication: msg_id=0x%x buf_len=%d pending_ind=%d\n", msg_id,ind_buf_len, pending_inds);
}
 ...
 )
{
  ...
  switch (msg_id)
  {
    ...
    case QMI_TEST_DATA_IND_REG_RESP_V01:
      printf("resp:QMI_TEST_DATA_IND_REG_RESP_V01.pending_async=%d\n",pending_async);
      break;
    default:
      break;
  }
...
)
{
  ...
  usleep(200);
  /* Wait until all pending async messages have been received */
  printf("pending_async=%d,pending_inds=%d\n",pending_async,pending_inds);
  while (pending_async != 0)
  {
    usleep(500);
  }
...
int main(int argc, char **argv)
{
  ...
  printf("release: qmi_client_release of clnt returned %d\n", rc);

  rc = qmi_client_release(notifier);
  printf("release: qmi_client_release of notifier returned %d\n", rc);
  return 0;
}
